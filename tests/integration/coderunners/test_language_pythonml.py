...
class TestPythonML:
    def test_hello_world(self):
        ...
        request = SubmissionRequest(test_cases=test_cases, language='pythonml', code={
            'main.py': dedent('''
                import numpy as np

                print(np.array(["Hello", "World!"])[0], np.array(["Hello", "World!"])[1])
            ''').strip(),
        })
    ...
    def test_mixed_type_output(self):
        ...
        request = SubmissionRequest(test_cases=test_cases, language='pythonml', code={
            'main.py': dedent('''
                ...
                print(*powers, "are powers of two")
            ''').strip(),
        }, return_outputs=True)
        res = CodeRunner.from_language(language=request.language).invoke(lambda_client, request=request)
        print(res)
    ...
    def test_compile_error(self):
        request = SubmissionRequest(test_cases=[], language='pythonml', code={
