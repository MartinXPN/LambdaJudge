...
class TestHaskell:
    def test_hello_world(self):
        ...
        request = SubmissionRequest(test_cases=test_cases, language='haskell', code={
            'Main.hs': dedent('''
                main :: IO ()
    ...
    def test_mixed_type_output(self):
        ...
        request = SubmissionRequest(test_cases=test_cases, language='hs', code={
            'main.hs': dedent('''
                main :: IO ()
                main = putStr $ unwords (map show [1, 2, 4, 8, 16, 32]) ++ " are powers of two"
            ''').strip(),
        }, return_outputs=True)
        res = CodeRunner.from_language(language=request.language).invoke(lambda_client, request=request)
        print(res)
    ...
    def test_compile_error(self):
        request = SubmissionRequest(test_cases=[], language='hs', code={
            'Main.hs': dedent('''
                main :: IO ()
                main = do
                    let value :: Int
